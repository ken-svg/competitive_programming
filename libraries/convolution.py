...
def _Number_Theorem_transformation(A, mod, root, inv = False):
  ...
  for b in range(B):
    ...
    dif >>= 1
  for _ in range(N - N_t):
    A.append(0)
  S = [A[i] for i in idx_set]  
  for _ in range(N - N_t):
    A.pop()
    ...
    if N == (section << 1): # 残り１回
      ...
      for base in range(0, N, section):
        for i in range(half):
          fac = factors[i]
          Sl = S[base | i]
          Sr = S[base | half | i] * fac
          S[base | i] = (Sl + Sr) % mod
          S[base | half | i] = (Sl - Sr) % mod
          # ↑バタフライ演算の本体
      ...
      for i in range(quarter):
        fac2 = (fac * fac) % mod
        fac3 = (fac2 * fac) % mod
        for base_i in range(i, N, section):
          Sill = S[base_i]
          Silr = S[base_i + quarter] * fac 
          Sirl = S[base_i + half] * fac2
          Sirr = S[base_i + half + quarter] * fac3
          S3 = (Silr - Sirr) % mod * imag
          
          S[base_i] = (Sill + Sirl + Silr + Sirr) % mod
          S[base_i + quarter] = (Sill - Sirl + S3) % mod
          S[base_i + half] = (Sill + Sirl - Silr - Sirr) % mod
          S[base_i + half + quarter] = (Sill - Sirl - S3) % mod
          # ↑バタフライ演算の本体
...
def _Number_Theorem_transformation_old(A, mod, root, inv = False):
  # 配列Aを最小の２べきに拡張(0fill)したもののフーリエ変換を、素数modを法とする有限体上で求める。
  # inv: Trueなら逆変換, mod:素数の法, root: 原始根
  # デフォルト値について 469762049 = 7*(2^26) + 1, 30^(2^26) = 1 より、2^26までのデータサイズを扱える。
  # 順変換、逆変換ともに【規格化していない】ので注意（順変換→逆変換と繰り返すと、len(A)以上最小の2べき倍された値が出てくる）
